...
**ASH** has two layers:
1. `include/ASH/` — clean abstract interfaces (`Device`, `Buffer`, `Texture`, `Pipeline`, etc.) that know nothing about Vulkan.
2. `src/vulkan/` — the concrete implementation of these interfaces on top of Vulkan (`VulkanDevice`, `VulkanBuffer`, etc.).

This separation makes it possible to add another backend (e.g. D3D12) in the future without touching code that uses `ASH`.
...
**GLFW is used only by the test app in `test/`** — as one possible example of integrating with a windowing library (you can use a different one). The only place where the RHI touches GLFW-related concepts is `VulkanDevice::attachSurface(VkSurfaceKHR)`, which simply accepts an already-created `VkSurfaceKHR` from anywhere.
...
#include "ASH/Buffer.h"   // or ASH/ASH.h for everything at once

ASH::vulkan::VulkanDevice device(/*enableValidation=*/true);
...
**Important:** resources that reference each other (`Framebuffer` holds pointers to `Texture`, `Pipeline` references `RenderPass`) must be destroyed in the reverse order of creation. See section 15 of `docs/USAGE.md` for details.
...
`VulkanSwapChain` keeps `kMaxFramesInFlight = 2` independent sets of synchronization primitives (`imageAvailable` semaphore + fence), letting the CPU prepare frame N+1 while the GPU is still drawing frame N. The `renderFinished` semaphores are indexed **by swapchain image index**, not by frame-in-flight slot — this is essential, otherwise there is a data race when the presentation engine reuses the semaphore.
...
`VulkanResult.h` throws a dedicated `ASH::DeviceLostException` (a `std::runtime_error` subclass) on `VK_ERROR_DEVICE_LOST`, distinguishable from ordinary errors. `VulkanDevice::isDeviceLost()` allows proactively checking device state without waiting for an exception.
