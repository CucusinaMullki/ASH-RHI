...
bool VulkanSwapChain::acquireNextImage(uint32_t& outImageIndex, ASH::Semaphore* signalSemaphore)
{
    auto* vulkanSemaphore = static_cast<VulkanSemaphore*>(signalSemaphore);
    VkSemaphore semaphoreHandle = vulkanSemaphore->getHandle();

    VkResult result = vkAcquireNextImageKHR(m_device, m_swapchain, UINT64_MAX,
        semaphoreHandle, VK_NULL_HANDLE, &outImageIndex);
...
void VulkanSwapChain::present(uint32_t imageIndex, ASH::Semaphore* waitSemaphore)
{
    auto* vulkanSemaphore = static_cast<VulkanSemaphore*>(waitSemaphore);
    VkSemaphore semaphoreHandle = vulkanSemaphore->getHandle();
